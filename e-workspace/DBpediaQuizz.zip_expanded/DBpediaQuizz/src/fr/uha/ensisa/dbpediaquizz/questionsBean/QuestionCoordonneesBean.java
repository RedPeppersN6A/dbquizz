...
public class QuestionCoordonneesBean extends QuestionBean {
	...
	public QuestionCoordonneesBean()
	{
		...
		double dist=1852*60*Math.acos(Math.sin(lat1)*Math.sin(lat2)+Math.cos(lat1)*Math.cos(lat2)*Math.cos(long1-long2));
		//1852: convertion de mille marin à metre
		...
		if(Math.random()<0.5)
		{
			...
			{	
				...
				if(reponseAbsente(ligne1.getLiteral("?pos").getString()))
				{
					this.mauvaisesReponses[index]= ((int)(Math.random()*2*10000000)) + "";//2*10^7 arc recouvrant la moitier de la terre
					index++;
				}
			}
		}
